...
const { username, muted } = await request.json().catch(() => ({}));
...
const value = muted ? Date.now() : null;
  await env.DB.prepare("UPDATE users SET muted_at = ? WHERE id = ?").bind(value, user.id).run();
